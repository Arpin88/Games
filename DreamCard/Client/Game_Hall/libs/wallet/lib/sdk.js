...
window.addEventListener('message', function(event){
	...
	switch(event.data.method)
	{
        ...
        case "onClose":
            ...
            break;
        case "onTransfer":
            onTransfer(event.data.data);
            break;
        case "onTransferResult":
            onTransferResult(event.data.data);
            break;
        case "onOffLine":
            onOffLine(event.data.data);
            break;
        case "onOffLineResult":
            onOffLineResult(event.data.data);
            break;
        case "onGetAddress":
            onGetAddress(event.data.data);
            break;
...
function onTransfer(data){
    XWG.Main.listenerCallback("transfer",data)
}
function onTransferResult(data){
    XWG.Main.listenerCallback("transferResult",data)
}
function onOffLine(data){
    XWG.Main.listenerCallback("offLine",data)
}
function onOffLineResult(data){
    XWG.Main.listenerCallback("offLineResult",data)
}
function onGetAddress(data){
    XWG.Main.listenerCallback("getAddress",data)
}

async function login(msg,walletName,account,callback)
{
    if(walletName == 'Metamask')
    {
        var web3 = new Web3(window.ethereum);
        web3.eth.personal.sign(msg,account,function(error,data){
            callback(data);
        })
    }else if(walletName == 'BinanceChain'){
        var data = await window.BinanceChain.request({method: "eth_sign", params: [account,msg]})
        callback(data)
    }
}
...
// var wallet_url = "http://47.242.166.72:3012/api-docs/";
// var wallet_url = "http://192.168.0.35:8080/";
...
(function (XWG) {
    ...
    class Main {
        ...
        start(url){
            ...
            Main.viewMgr.mainView.createIframe(url);
            Main.wallet_url = url;
        }
        login(msg,callback = null)
        {
            // document.getElementById("iframe").contentWindow.postMessage({method: "login",data:msg},wallet_url);
            // this.showMain();
        ...
        setAddress(address)
        {
            document.getElementById("iframe").contentWindow.postMessage({method: "setAddress",data:address},Main.wallet_url);
        }
        disconnect()
        {
            document.getElementById("iframe").contentWindow.postMessage({method: "disConnect"},Main.wallet_url);
        }
        showMain() {
...
(function(XWG){
	class SDK{
        ...
        static async login(msg,walletName,callback = null) {
            var accounts = [];
            if(walletName == 'Metamask')
            {
                if(typeof window.ethereum == 'undefined')
                {
                    callback({result:1,msg:-20000})
                    return;
                }
                ...
                {
                    callback({result:1,msg:-20001})
                    return;
                }
                accounts = await window.BinanceChain.request({method: 'eth_requestAccounts'})
            }
            login(msg,walletName,accounts[0],function(data){
                if(data)
                {
                    callback({result:0,sign:data,account:accounts[0]});
                }else{
                    callback({result:1,msg:-20002})
                }
