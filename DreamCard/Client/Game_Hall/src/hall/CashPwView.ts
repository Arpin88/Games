// TypeScript file
class CashPwView extends BaseView{
    public static NAME:string = "CashPWSkin";

    public constructor(){
        super(CashPwView.NAME);
    }
//
    ...
    private touchTap(event:egret.TouchEvent):void{
        ...
        if(tar instanceof eui.Button){
            if(tar==self.btnConfirm){
              ...
              if(self.isNumber(this.editCount.text) == false  || this.editCount.text.length != 6){
                ...
                if(this.func != null)
                {
                    this.func(self.editCount.text,this.cashNum,this.cashOil,this.addr);
                }
